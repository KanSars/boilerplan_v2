...
import { equipmentMark, pointColor, shortEquipmentName } from "../../shared/formatting/boilerRoomFormatters";
import { EquipmentPlanSymbol } from "./EquipmentPlanSymbol";
...
export const PlanCanvas = ({
  ...
  const room = { widthMm: 6000, lengthMm: 4500 };
  const margin = 600;
  ...
  return (
    <div ref={viewportRef} className="zoomViewport">
      <div className="zoomContent" style={{ width: `${zoom * 100}%`, height: `${zoom * 100}%` }}>
        <svg ref={svgRef} className="planCanvas" viewBox={planViewBox} role="img" aria-label="План помещения" onPointerDown={startPan} onPointerMove={move} onPointerUp={stopPointer} onPointerCancel={stopPointer}>
          <defs>
          ...
          {routes.map((route) => route.points.length > 1 && (
            <polyline key={route.id} points={route.points.map((point) => `${origin.x + point.xMm},${origin.y + point.yMm}`).join(" ")} fill="none" stroke={route.medium === "natural_gas" ? "#a16207" : route.id.includes("t2") ? "#1d4ed8" : "#b91c1c"} strokeWidth="70" strokeLinecap="round" strokeLinejoin="round" strokeDasharray={route.status === "needs_confirmation" ? "160 110" : undefined} />
          ))}
          {equipment.map((item) => {
            ...
            return (
              <g key={item.instance.id} onPointerDown={(event) => startDrag(event, item)} className={selected ? "selectedSvg draggable" : "draggable"}>
                ...
                <text x={x + width / 2} y={y + height / 2 + 45} textAnchor="middle" fontSize="120" fontWeight="700">{equipmentMark(item)}</text>
